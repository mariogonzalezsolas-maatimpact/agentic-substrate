...
├── validators/       ───┤    (manifest-driven)                (selective copy)
├── metrics/          ───┤
└── templates/        ───┘    update.sh                        .agentic-substrate-version
...
# Expected output: "⚠️  Modified files detected: agents/chief-architect.md"
...
**Installation**:
- ✅ install.sh uses manifest-driven copying (no `cp -r`)
- ✅ install.sh detects existing installation
- ✅ install.sh creates version file
- ✅ install.sh generates manifest
- ✅ install.sh generates rollback script
- ✅ install.sh validates installation
- ✅ All 35 managed files installed correctly
- ✅ Permissions set on all .sh files
- ✅ Protected files never touched

**Updates**:
- ✅ update.sh detects current version
- ✅ update.sh identifies changed files
- ✅ update.sh detects user modifications
- ✅ update.sh updates only changed files
- ✅ update.sh preserves user data
- ✅ update.sh updates manifest and version
...
- ✅ User modifications preserved or warned

**Repository Hygiene**:
- ✅ User-specific files removed from git
- ✅ .gitignore prevents future commits
- ✅ manifest-template.json is source of truth
...
✅ **Plan ready for @code-implementer or manual implementation**
